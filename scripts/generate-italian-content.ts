// scripts/generate-italian-content.ts
//
// One-off/reproducible build step: parses the plain-text Italian curriculum in
// content-source/italian/*.txt (structure documented in
// italian_00_curriculum_index.txt — "####" = part divider, "====" = lesson
// divider, "ALL-CAPS LABEL:" = section) into self-contained per-lesson HTML
// files under content/italian/, plus manifest-italian.json describing the
// classroom/unit/lesson structure for prisma/seed-italian.ts.
//
// Run with: npx tsx scripts/generate-italian-content.ts

import fs from "node:fs";
import path from "node:path";

const SRC_DIR = path.join(process.cwd(), "content-source", "italian");
const OUT_DIR = path.join(process.cwd(), "content", "italian");
const MANIFEST_PATH = path.join(process.cwd(), "manifest-italian.json");

const SOURCE_FILES = [
  "italian_01_foundations.txt",
  "italian_02_everyday_life.txt",
  "italian_03_food_shopping_culture.txt",
  "italian_04_travel_practical.txt",
  "italian_05_advanced_wrapup.txt",
];
...
// "Italian sentence - phonetic - English translation".
function renderExamples(text: string): string {
  ...
  const blocks = entries.map((entry) => {
    ...
    return `<div class="example"><p class="it">${escapeHtml(it)}</p><span class="pron">${escapeHtml(pron)}</span><p class="en">${escapeHtml(en)}</p></div>`;
  });
  ...
  const manifestUnits = parts.map((part) => ({
    ...
    lessons: part.lessons.map((lesson) => {
      ...
      return {
        ...
        file: `content/italian/${fileName}`,
      };
    }),
  }));

  const manifest = {
    classroom: { language: "Italian", level: "Beginner–Intermediate" },
    units: manifestUnits,
  };
